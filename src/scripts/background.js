...
function sendDesktopNotification(feeds) {
  function createNotifications(feedsToNotifications, showBlogIcons, showThumbnails) {
    /* eslint-disable no-restricted-syntax */
...
function openFeedlyTab() {
  browser.tabs.query({ url: `${appGlobal.feedlyUrl}/*` }).then((tabs) => {
    if (tabs.length < 1) {
...
function getOptions() {
  ...
  return new Promise((resolveOptions) => {
    appGlobal.syncStorage.get(null, (items) => {
      ...
      // @if BROWSER=='chrome'
      const getBackgroundPermissionPromise = new Promise((resolveBackgroundMode) => {
        try {
      ...
      // @if BROWSER!='firefox'
      const getAllSitesPermissionPromise = new Promise((resolveAllSitesPermission) => {
...
function getUserSubscriptions(updateCache) {
  ...
  appGlobal.getUserSubscriptionsPromise =
    appGlobal.getUserSubscriptionsPromise
    ||
    apiRequestWrapper('subscriptions').then((response) => {
      if (!response) {
        appGlobal.getUserSubscriptionsPromise = null;
        return Promise.reject();
      }
...
function makeMarkersRequest(parameters) {
  apiRequestWrapper('markers/counts', { parameters }).then((markersResponse) => {
...
function updateFeeds(silentUpdate) {
  ...
  return Promise.all(promises).then((responses) => {
    ...
    if (!silentUpdate
      ...
      filterByNewFeeds(appGlobal.cachedFeeds, (newFeeds) => {
...
function updateSavedFeeds() {
  return apiRequestWrapper(`streams/${encodeURIComponent(appGlobal.savedGroup)}/contents`)
    .then(response => parseFeeds(response))
    .then((feeds) => {
